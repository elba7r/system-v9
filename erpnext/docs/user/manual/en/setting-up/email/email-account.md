...
You can manage multiple incoming and outgoing Email Accounts in Revalue ERP. There has to be atleast one default outgoing account and one default incoming account. If you are on the Revalue ERP cloud, the default outgoing email is set by us.
...
In Revalue ERP when you send an email to a contact like a customer, the sender will be the user who sent the email. In the **Reply-To** property, the email id will be of the default incoming account (like `replies@yourcompany.com`). Revalue ERP will automatically extract these emails from the incoming account and tag it to the relvant communication
