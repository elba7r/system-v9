...
class TestDelivery(unittest.TestCase):
	...
	def test_delivery(self):
		if not frappe.db.exists("Delivery", "TOUR-00000"):
			delivery = frappe.get_doc("Delivery")
			delivery.name = "TOUR-00000"
			...
			notify_customers(docname = delivery.name, date = delivery.date, driver = delivery.driver, vehicle = delivery.vehicle,
				sender_email = sender_email, delivery_notification = delivery.delivery_notification)
